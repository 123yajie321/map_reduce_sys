...
public class CVM extends AbstractCVM {
	...
	public void deploy() throws Exception {
		...
		Function<Tuple, Tuple> f_map = a -> { 
				ArrayList<Tuple> data=(ArrayList<Tuple>)a.getIndiceData(0);
				for(int i=0;i<data.size();i++) {
					Double resDouble =(Double)data.get(i).getIndiceData(0)*10.0;
					Tuple tuple = new Tuple(1);
					tuple.setIndiceTuple(0, resDouble); 
					data.set(i,tuple);
				}
				Tuple result=new Tuple(1);
				result.setIndiceTuple(0, data);
				 return result; 
				 
		};
		
		BiFunction<Tuple, Tuple, Tuple> g_reduce = (acc,y) -> { 
				BiFunction<Tuple, Tuple, Tuple> g = (a,b) -> { Double resDouble =
					(Double)a.getIndiceData(0)+(Double)b.getIndiceData(0);
					Tuple tuple = new Tuple(1); tuple.setIndiceTuple(0, resDouble); 
					return tuple;
				};
				Double cpt=(Double)acc.getIndiceData(0);
				ArrayList<Tuple>tuples=(ArrayList<Tuple>)y.getIndiceData(0);
		
				
				for(int i=0;i<tuples.size()-1;i++){
					
					Tuple tmp=g.apply(tuples.get(i), tuples.get(i+1));
					tuples.remove(i);
					tuples.remove(i);
					tuples.add(tmp);
					i=i-1;
				...
				Tuple reult=new Tuple(1);
				reult.setIndiceTuple(0, tuples.get(0));
				return reult;
	   ...
	   //  System.out.println(cmURI+" ; "+ComponentGestion.GMOP_URI+" ; "+ComponentMap.MGIP_URI+" ; "+ConnectorMapGestion.class.getCanonicalName());
		
		this.doPortConnection(cmURI,ComponentGestion.GMOP_URI,ComponentMap.MGIP_URI ,ConnectorMapGestion.class.getCanonicalName());
		Object[] param = new Object[1];
		paramObjects[0] = g_reduce;
		String reduceUriString=AbstractComponent.createComponent(ComponentReduce.class.getCanonicalName(),param);
		this.doPortConnection(reduceUriString, ComponentMap.MSROP_URI, ComponentReduce.RRMIP_URI, ConnectorMapGestion.class.getCanonicalName());
		super.deploy();
	}
